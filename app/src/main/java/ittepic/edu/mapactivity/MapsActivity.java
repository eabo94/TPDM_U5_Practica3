...
if(listaPuntos.size() > 0) {
    ...
    //.icon(BitmapDescriptorFactory.fromResource(R.drawable.marker));
        Marker marcador_= mMap.addMarker(new MarkerOptions().position(new LatLng((Double.parseDouble(listaPuntos.get(i).getLatitud())), (Double.parseDouble(listaPuntos.get(i).getLongitud()))))
                //   .icon(BitmapDescriptorFactory.fromResource(R.drawable.marker))
...
VolverPosicion(ultpos);
    ...
    public boolean onMarkerClick(Marker marker) {
        try{Integer clickCount = (Integer) marker.getTag();
            return true;
        }catch (Exception ex) {
       ...
       // String url__img = info.getImagenLugar();
        //Glide.with(this).load(url__img)
       //Config.URL_IMAGES_PUBLIC + url__img+".jpg")
